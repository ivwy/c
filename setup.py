...
async def on_ready():
    banner()
    print((Fore.GREEN + Style.BRIGHT) + f"[INFO][SYSTEM]: You have logged in as {client.user.name}.")
    ...
    print(f"[INFO][SYSTEM]: OS Information {platform.platform()}.")
...
for file in os.listdir(f"./cogs"):
    if file.endswith(".py"):
        ...
        try:
            ...
            print((Fore.GREEN + Style.BRIGHT) + f"[INFO][SYSTEM]: Succesfully loaded {extension}.")
        except Exception as e:
            exception = f"{type(e).__name__}: {e}"
            print((Fore.RED + Style.BRIGHT) + f"[INFO][SYSTEM]: Failed to load extension {extension}\n{exception}.")

try:
    client.run(token)
except Exception as error:
    print((Fore.RED + Style.BRIGHT) + f"[INFO][SYSTEM]: Error when logging in: {error}.")